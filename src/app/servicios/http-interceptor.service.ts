 ...
 */
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    const tokenEstablecimiento: string = this.autenticadorJwt.recuperaJWTEstablecimiento();  
    if (tokenEstablecimiento) {
      request = request.clone({ headers: request.headers.set('AuthorizationEstablecimiento', 'Bearer ' + tokenEstablecimiento) });
    }

    const tokenPeluquqero: string = this.autenticadorJwt.recuperaJWTPeluquero();  
    if (tokenPeluquqero) {
      request = request.clone({ headers: request.headers.set('AuthorizationPeluquero', 'Bearer ' + tokenPeluquqero) });
    }
